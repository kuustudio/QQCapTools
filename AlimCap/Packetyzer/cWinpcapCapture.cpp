...
BOOL cWinpcapCapture::CapturePackets(LPVOID uParam, packet_call_handler pFunc, UINT AdapterIndex, UINT MaxNumOfPackets, const CHAR* Filter = NULL)
{
	...
	while ((retValue = pcap_next_ex(fp, &PacketHeader, &PacketData)) >= 0 && n < MaxNumOfPackets)
	{
		...
		if (dwRetVal == WAIT_TIMEOUT)
		{
			...
			if (retValue == 0)
				continue;
			n++;
			pFunc(uParam, PacketData, PacketHeader->len);
			nCapturedPackets++;
		}
		else
		{
...
BOOL cWinpcapCapture::StopCapture()
{
